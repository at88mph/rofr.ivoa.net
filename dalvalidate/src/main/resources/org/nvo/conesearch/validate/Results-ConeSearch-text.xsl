...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
   ...
   <xsl:template match="/">
Cone Search Verification Results
...
The following <xsl:value-of select="count(ConeSearchValidation/testQuery)"/> test queries were sent to the service:

<xsl:for-each select="ConeSearchValidation/testQuery">  o  <xsl:value-of select="@name"/>: <xsl:value-of select="@options"/><xsl:text>
</xsl:text> 
</xsl:for-each>
   </xsl:otherwise>
</xsl:choose>
...
Description:     test response from a position near the Orion nebula. </xsl:when>
         <xsl:when test="@role='sr0'">
Description:     test response to a metadata query where SR=0. </xsl:when>
