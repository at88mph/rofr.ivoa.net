...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
   ...
   <xsl:template match="/" mode="query">
      ...
      <xsl:apply-templates select="//RESOURCE[1]/TABLE[1]" mode="tests"/>
   ...
   <xsl:template name="T2.1a">
      <xsl:variable name="stat">
         <xsl:copy-of select="count(//RESOURCE)=1"/>
      </xsl:variable>
   ...
   <xsl:template name="T2.1b">
      <xsl:variable name="stat">
         <xsl:copy-of select="count(//RESOURCE/TABLE)=1"/>
      </xsl:variable>
      <xsl:call-template name="reportResult">
   ...
   <xsl:template match="TABLE" mode="tests">
      ...
      <xsl:apply-templates select="FIELD|/VOTABLE/DEFINITIONS/PARAM" 
   ...
   <xsl:template name="hasUCD">
      ...
      <xsl:variable name="stat">
         <xsl:copy-of select="count(FIELD[@ucd=$ucd])=1"/>
      </xsl:variable>
      ...
      <xsl:for-each select="FIELD[@ucd=$ucd]">
   ...
   <xsl:template name="T2.2.2">
      ...
      <xsl:apply-templates select="FIELD[@ucd='POS_EQ_RA_MAIN']" 
                           mode="checkDegUnit">
   ...
   <xsl:template name="T2.2.3">
      ...
      <xsl:apply-templates select="FIELD[@ucd='POS_EQ_DEC_MAIN']" 
                           mode="checkDegUnit">
   ...
   <xsl:template match="FIELD" mode="checkDegUnit">
      <xsl:param name="item">??</xsl:param>
   ...
   <xsl:template name="T2.2.4">
      <xsl:variable name="stat">
         <xsl:copy-of select="not(FIELD[@ucd='OBS_ANG-SIZE'] and 
                           /VOTABLE/DEFINITIONS/PARAM[@ucd='OBS_ANG-SIZE'])"/>
      </xsl:variable>
      ...
      <xsl:for-each select="FIELD[@name='PositionalError'] |
                         /VOTABLE/DEFINITIONS/PARAM[@name='PositionalError']"> 
      ...
      <xsl:for-each select="FIELD[@ucd='OBS_ANG-SIZE'] |
                           /VOTABLE/DEFINITIONS/PARAM[@ucd='OBS_ANG-SIZE']">
   ...
   <xsl:template name="T2.2.1d">
      ...
      <xsl:variable name="match">
         <xsl:for-each 
              select="DATA/TABLEDATA/TR">
            <xsl:variable name="row">
               <xsl:value-of select="position()"/>
            </xsl:variable>
            <xsl:variable name="val" select="TD[position()=number($colnum)]"/>
            <xsl:choose>
               <xsl:when test="../TR[position()!=number($row)]/TD[position()=number($colnum) and .=$val]">0</xsl:when>
   ...
   <!--
     ...
     -->
   <xsl:template match="FIELD|PARAM" mode="checkFieldAnnot">
   ...
   <xsl:template name="T2.3">
      <xsl:variable name="stata">
         <xsl:copy-of select="count(/VOTABLE/DESCRIPTION)>0"/>
      </xsl:variable>
      ...
      <xsl:variable name="statb">
         <xsl:copy-of select="count(/VOTABLE/RESOURCE/TABLE/DESCRIPTION)>0"/>
      </xsl:variable>
      ...
      <xsl:if test="$queryType='sr0'">
         <xsl:variable name="statc">
            <xsl:copy-of 
                 select="not(/VOTABLE/RESOURCE/TABLE/DATA/TABLEDATA/TR)"/>
         </xsl:variable>
   ...
   <xsl:template name="T3">
      <xsl:variable name="stata">
         <xsl:copy-of 
              select="count(/VOTABLE/INFO[@name='Error'] |
                            /VOTABLE/RESOURCE/INFO[@name='Error'] |
                            /VOTABLE/DEFINITIONS/PARAM[@name='Error']     |
                            /VOTABLE/RESOURCE/PARAM[@name='Error'] )=1"/>
      </xsl:variable>
      ...
      <xsl:variable name="statb">
         <xsl:copy-of select="count(//PARAM | //INFO)=1"/>
      </xsl:variable>
...
<!--
  ...
  -
      <xsl:variable name="statc">
         <xsl:copy-of select="count(/VOTABLE/RESOURCE)=0"/>
      </xsl:variable>
