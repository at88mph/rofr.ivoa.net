...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
   ...
   <!--
     ...
     -    meta    metadata query triggered by SR=0
     -    error   an erroneous query meant to result in an error response
     -  </pre>
     -->
   <xsl:param name="query">area</xsl:param>
   ...
   <xsl:template name="T4.2.4d">
      ...
      <xsl:apply-templates select="PARAM[@ucd=$ucd]|
                                   TABLE/FIELD[@ucd=$ucd]" 
                           mode="checkDegUnit">
         <xsl:with-param name="item">4.2.4d</xsl:with-param>
         <xsl:with-param name="ucd" select="$ucd"/>
   ...
   <xsl:template name="T4.2.4y">
      <xsl:apply-templates select="PARAM[@name!='QUERY_STATUS' and 
   ...
   <xsl:template match="FIELD|PARAM" mode="flddesc">
      ...
      <xsl:if test="@datatype='double' or @datatype='float' or 
         ...
         <xsl:call-template name="reportResult">
            <xsl:with-param name="item">T4.2.4y/b</xsl:with-param>
   ...
   <xsl:template name="T4.3.1a">
      <xsl:variable name="stat">
         <xsl:copy-of select="count(INFO[@name='QUERY_STATUS' and 
                                         @value='ERROR'])=1"/>
      </xsl:variable>
      <xsl:call-template name="reportResult">
   ...
   <xsl:template name="T4.3b">
      ...
      <xsl:call-template name="reportResult">
         ...
         <xsl:with-param name="desc">
            ...
            <xsl:text>with QUERY_STATUS='ERROR'.</xsl:text>
         </xsl:with-param>
      </xsl:call-template>
   </xsl:template>   
   ...
   <xsl:template name="checkInputParams">
      <!-- check for require input PARAMs (POS, SIZE, FORMAT) -->
   ...
   <xsl:template name="T7.1e">
      <xsl:variable name="stat">
         <xsl:value-of select="PARAM[@name='INPUT:FORMAT' and VALUES]"/>
      </xsl:variable>
...
<!-- ========================================================== -->
   ...
   <xsl:template name="hasUCD">
      ...
      <xsl:variable name="stat">
         <xsl:copy-of select="count(PARAM[@ucd=$ucd]|
      ...
      <xsl:for-each select="PARAM[@ucd=$ucd]|
                            TABLE/FIELD[@ucd=$ucd]">
         ...
         <xsl:variable name="asstat">
           <xsl:choose>
              <xsl:when test="$arraysize=''">
                 <xsl:copy-of select="boolean('true')"/>
              </xsl:when>
              <xsl:otherwise>
                 <xsl:copy-of select="@arraysize=$arraysize"/>
              </xsl:otherwise>
           </xsl:choose>
         </xsl:variable>
         ...
         <xsl:call-template name="reportResult">
            ...
            <xsl:with-param name="desc">
               <xsl:text>The </xsl:text>
               <xsl:value-of select="$ucd"/>
               <xsl:text> column must have arraysize='</xsl:text>
