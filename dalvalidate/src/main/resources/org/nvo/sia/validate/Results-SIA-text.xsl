...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
   ...
   <xsl:template match="/">
Simple Image Access Verification Results
...
The following <xsl:value-of select="count(SIAValidation/testQuery)"/> test queries were sent to the service:

<xsl:for-each select="SIAValidation/testQuery">  o  <xsl:value-of select="@name"/>: <xsl:value-of select="@options"/><xsl:text>
</xsl:text> 
</xsl:for-each>
   </xsl:otherwise>
</xsl:choose>
...
Description:     test response from the user-provided query except
                 with only one SIZE value provided. </xsl:when>
