...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
   ...
   <!-- 
     - the title of the SLA resource
     -->
   <xsl:param name="title" select="''"/>

   <!-- 
     - the short name of the SLA resource
     -->
   ...
   <!--
     ...
     -->
   <xsl:param name="resultsRootElement">SimpleLineAccess</xsl:param>
   ...
   <xsl:template match="/">
      ...
      <xsl:element name="{$resultsRootElement}">
         <xsl:attribute name="votable-version">
            <xsl:value-of select="$dtdver"/>
         ...
         <xsl:attribute name="recordCount">
            <xsl:value-of select="count(//RESOURCE[1]/TABLE//TABLEDATA/TR)"/>
         </xsl:attribute>
   ...
   <xsl:template match="/" mode="query">
      ...
      <xsl:call-template name="T6.0.1a"/>

      <!-- recommend that the results be in the first RESOURCE -->
      <xsl:call-template name="T6.0.1c"/>

      <xsl:choose>
         <xsl:when test="//RESOURCE[@type='results']">
            <xsl:apply-templates select="//RESOURCE[@type='results']" 
                                 mode="tests"/>
         </xsl:when>
         <xsl:when test="//RESOURCE[1]">
            <xsl:apply-templates select="//RESOURCE[1]" mode="tests"/>
         </xsl:when>
      </xsl:choose>
   </xsl:template>
   ...
   <!--
     ...
     -->
   <xsl:template match="RESOURCE" mode="tests">

      <!-- make sure there is only one TABLE in your RESOURCE of results -->
      <xsl:call-template name="T6.0.1b"/>

      <!-- is there an INFO element? -->
      <xsl:call-template name="T6.0.2a"/>

      <xsl:for-each select="INFO[@name='QUERY_STATUS']">

         <!-- is a legal query supported? -->
         <xsl:call-template name="T4.0"/>
      </xsl:for-each>
   ...
   <!--
     -  ensure there is a RESOURCE with type=results
     -->
   <xsl:template name="T6.0.1a">
      <xsl:variable name="stat">
         <xsl:copy-of select="count(//RESOURCE[@type='results'])=1"/>
      </xsl:variable>
      <xsl:call-template name="reportResult">
         <xsl:with-param name="item">6.0.1a</xsl:with-param>
   ...
   <!--
     -  recommend that the results be in the first RESOURCE
     -->
   <xsl:template name="T6.0.1c">
      <xsl:variable name="stat">
         <xsl:copy-of select="boolean(//RESOURCE[position()=1 and 
                                                 @type='results'])"/>
      </xsl:variable>
      <xsl:call-template name="reportResult">
         <xsl:with-param name="item">6.0.1c</xsl:with-param>
         <xsl:with-param name="status" select="$stat"/>
   ...
   <!--
     ...
     - @context RESOURCE
     -->
   <xsl:template name="T6.0.1b">
      <xsl:variable name="stat">
         <xsl:copy-of select="count(TABLE)=1"/>
      </xsl:variable>
      <xsl:call-template name="reportResult">
         <xsl:with-param name="item">6.0.1b</xsl:with-param>
         <xsl:with-param name="status" select="$stat"/>
         <xsl:with-param name="desc">
   ...
   <!--
     ...
     - @context RESOURCE
     -->
   <xsl:template name="T6.0.2a">
      <xsl:variable name="stat">
         <xsl:copy-of select="count(INFO[@name='QUERY_STATUS'])=1"/>
      </xsl:variable>
      <xsl:call-template name="reportResult">
         <xsl:with-param name="item">6.0.2a</xsl:with-param>
         <xsl:with-param name="status" select="$stat"/>
         <xsl:with-param name="desc">
...
-->
   ...
   <!--
     ...
     - @context INFO[@name='QUERY_STATUS']
     -->
   <xsl:template name="T4.0">
      <xsl:param name="qtype">Legal</xsl:param>
      <xsl:param name="ref">4.0</xsl:param>
   ...
   <xsl:template name="checkFields">
      <xsl:call-template name="T6.1a"/>
      <xsl:call-template name="T6.1b"/>
      <xsl:call-template name="T6.1d"/>
      <xsl:call-template name="T6.1e"/>
      <xsl:call-template name="T6.1f"/>
   </xsl:template>

   <!-- Line Metadata -->

   <xsl:template name="T6.1a">
      <xsl:call-template name="hasUCD">
         <xsl:with-param name="item">6.1a</xsl:with-param>
         <xsl:with-param name="ucd">em.wl</xsl:with-param>
         <xsl:with-param name="type">double</xsl:with-param>
      </xsl:call-template>
   </xsl:template>

   <xsl:template name="T6.1b">
      <xsl:call-template name="hasUCD">
         <xsl:with-param name="item">6.1b</xsl:with-param>
         <xsl:with-param name="ucd">em.line</xsl:with-param>
         <xsl:with-param name="arraysize">*</xsl:with-param>
      </xsl:call-template>
   </xsl:template>

   <xsl:template name="T6.1d">
      <xsl:call-template name="hasUCD">
         <xsl:with-param name="item">6.1d</xsl:with-param>
         <xsl:with-param name="ucd">phys.atmol.element</xsl:with-param>
         <xsl:with-param name="level">rec</xsl:with-param>
         <xsl:with-param name="arraysize">*</xsl:with-param>
      </xsl:call-template>
   </xsl:template>

   <xsl:template name="T6.1e">
      <xsl:call-template name="hasUCD">
         <xsl:with-param name="item">6.1e</xsl:with-param>
         <xsl:with-param name="ucd">phys.atmol.initial;phys.atmol.level</xsl:with-param>
         <xsl:with-param name="level">rec</xsl:with-param>
         <xsl:with-param name="arraysize">*</xsl:with-param>
      </xsl:call-template>
   </xsl:template>

   <xsl:template name="T6.1f">
      <xsl:call-template name="hasUCD">
         <xsl:with-param name="item">6.1f</xsl:with-param>
         <xsl:with-param name="ucd">phys.atmol.final;phys.atmol.level</xsl:with-param>
         <xsl:with-param name="level">rec</xsl:with-param>
         <xsl:with-param name="arraysize">*</xsl:with-param>
      </xsl:call-template>
   </xsl:template>
   ...
   <xsl:template name="T4.2.4y">
      <xsl:apply-templates select="PARAM[not(starts-with(@name,'INPUT:'))] |
                                   TABLE/FIELD" 
                           mode="flddesc" />
   </xsl:template>

   <xsl:template match="FIELD|PARAM" mode="flddesc">
      <xsl:variable name="name">
         <xsl:call-template name="fieldlabel"/>
      </xsl:variable>

      <xsl:variable name="stat">
         <xsl:value-of select="@datatype and @ucd and DESCRIPTION"/>
      </xsl:variable>
   ...
   <xsl:template match="/" mode="error">
      ...
      <xsl:choose>
         ...
         <xsl:when test="//RESOURCE[1]">
            <xsl:apply-templates select="//RESOURCE[1]" mode="error"/>
         </xsl:when>
      </xsl:choose>
   </xsl:template>
   ...
   <!--
     ...
     -->
   <xsl:template match="RESOURCE" mode="error">
      <xsl:call-template name="T6.0.2a"/>
      <xsl:for-each select="INFO[@name='QUERY_STATUS']">
...
-->
   ...
   <xsl:template match="/" mode="metadata">
      ...
      <xsl:choose>
         ...
         <xsl:when test="//RESOURCE[1]">
            <xsl:apply-templates select="//RESOURCE[1]" mode="metadata"/>
         </xsl:when>
      </xsl:choose>
   </xsl:template>
   ...
   <!--
     ...
     -->
   <xsl:template match="RESOURCE" mode="metadata">
      ...
      <xsl:for-each select="INFO[@name='QUERY_STATUS']">
         ...
         <xsl:call-template name="T4.0">
            ...
            <xsl:with-param name="ref">5.1</xsl:with-param>
         </xsl:call-template>
      </xsl:for-each>
   ...
   <xsl:template name="checkInputParams">
      <!-- check for required input PARAMs (WAVELENGTH) -->
      <xsl:call-template name="T5.1a"/>

      <!-- recommend documenting other input PARAMs -->
      <xsl:call-template name="T5.1b"/>

      <!-- recommend descriptions for input PARAMs -->
      <xsl:call-template name="T5.1c"/>
   </xsl:template>
   ...
   <!-- 
     ...
     -  @context RESOURCE
     -->
   <xsl:template name="T5.1a">
      <xsl:call-template name="checkReqInputParam">
         <xsl:with-param name="inp">WAVELENGTH</xsl:with-param>
      </xsl:call-template>
   </xsl:template>
   ...
   <xsl:template name="checkReqInputParam">
      ...
      <xsl:variable name="stat">
         <xsl:copy-of select="PARAM[@name=concat('INPUT:',$inp)]"/>
      </xsl:variable>

      <xsl:call-template name="reportResult">
         <xsl:with-param name="item">5.1a</xsl:with-param>
         <xsl:with-param name="status" select="$stat"/>
         <xsl:with-param name="desc">
   ...
   <!-- 
     ...
     -  @context RESOURCE
     -->
   <xsl:template name="T5.1c">
      <xsl:apply-templates select="PARAM[starts-with(@name,'INPUT:')]" 
                           mode="desc"/>
   </xsl:template>  

   <xsl:template match="FIELD|PARAM" mode="desc">
      <xsl:variable name="name">
      ...
      <xsl:call-template name="reportResult">
         <xsl:with-param name="item">5.1c</xsl:with-param>
         <xsl:with-param name="status" select="$stat"/>
         <xsl:with-param name="type">rec</xsl:with-param>
         <xsl:with-param name="desc">
   ...
   <!--
     -  recommend documenting other input PARAMs
     -->
   <xsl:template name="T5.1b">
      <xsl:variable name="stat">
         <xsl:value-of select="PARAM[starts-with(@name,'INPUT:') and 
                                     @name != 'INPUT:WAVELENGTH']"/>
      </xsl:variable>
      
      <xsl:call-template name="reportResult">
         <xsl:with-param name="item">5.1b</xsl:with-param>
         <xsl:with-param name="status" select="$stat"/>
         <xsl:with-param name="type">rec</xsl:with-param>
         <xsl:with-param name="desc">
...
<!-- ========================================================== -->
   ...
   <xsl:template name="hasUCD">
      ...
      <xsl:variable name="exactCase">
         <xsl:copy-of select="count(PARAM[@ucd=$ucd and 
                                          not(starts-with(@name, 'INPUT:'))] |
                                    TABLE/FIELD[@ucd=$ucd])=1"/>
      </xsl:variable>

      <xsl:variable name="anyCase">
        <xsl:copy-of select="count(PARAM[translate(@ucd,
                                    'abcdefghijklmnopqrstuvwxyz',
                                    'ABCDEFGHIJKLMNOPQRSTUVWXYZ')=$upucd and 
                                         not(starts-with(@name, 'INPUT:'))] |
                                   TABLE/FIELD[translate(@ucd,
                                    'abcdefghijklmnopqrstuvwxyz',
      ...
      <xsl:for-each select="PARAM[translate(@ucd,
                                    'abcdefghijklmnopqrstuvwxyz',
                                    'ABCDEFGHIJKLMNOPQRSTUVWXYZ')=$upucd and 
                                  not(starts-with(@name, 'INPUT:'))] |
                            TABLE/FIELD[translate(@ucd,
   ...
   <!--
     ...
     -->
   <xsl:template match="FIELD|PARAM" mode="checkDegUnit">
      <xsl:param name="item">??</xsl:param>
