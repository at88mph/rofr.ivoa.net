...
<html> <head>
  <title>SLA Validation Results</title>
  ...
  <tr>
    ...
    <td valign="top">
      <table  width="100%" border="0" cellpadding="0" cellspacing="0">
       ...
       <!-- Name of Application -->
       Simple Line Access Service Validater
       ...
       <tr>
         ...
         <table cellspacing="2" cellpadding="0" border="0" width="100%"
           ...
           <tr>
             <!-- the local links -->
             <td class="navlink"><a href="http://www.ivoa.net/Documents/SLAP/">SLA Standard</a></td>
...
<h1>Simple Line Access Service Verification Results</h1>
...
<xsl:choose>
   <xsl:when test="count(SLAValidation/testQuery)=1">
A single test query (<xsl:value-of 
   select="SLAValidation/testQuery/@options"/>) was sent to
the service.  The results are described below.  Click on the links to
   see the part of the SLA Specification relevant to that test
   result.  <p />
   </xsl:when>
   <xsl:otherwise>
The following 
<xsl:value-of select="count(SLAValidation/testQuery)"/> 
test queries were sent to the service:
<ul>
      <xsl:for-each select="SLAValidation/testQuery">
...
<xsl:apply-templates select="SLAValidation/testQuery"/>
   ...
   <xsl:template match="testQuery">
      ...
      <xsl:choose>
         ...
         <xsl:when test="@name='user'">
            ...
            user-provided query (assuming wavelength is within service's
            coverage.)  <br />
         </xsl:when>
         ...
         <xsl:when test="@name='badpos'">
            ...
            query (with a non-decimal wavelength value). <br />
         </xsl:when>
      ...
      <xsl:choose>
         ...
         <xsl:when test="@role='error'">
             <a href="sla.html#query-err">error handling</a>
         </xsl:when>
      ...
      <xsl:choose>
         <xsl:when test="test[@status='fail']">
            ...
            SLA standard and correct the errors to be fully
            compliant. <p />
      ...
      <xsl:choose>
         <xsl:when test="test[@status='warn']">
            ...
            SLA standard document for clarification. <p />
