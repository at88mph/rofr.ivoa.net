...
/**
 * a validation session handler for SIA services.
 */
public class SIAValidationSession 
    extends SimpleIVOAServiceValidationSession 
{
     ...
     */
    public SIAValidationSession() throws IOException, SAXException {
        super();
    }
     ...
     *                configuration file 
     */
    public SIAValidationSession(TransformerFactory tf, Class resClass) 
     ...
     *                configuration file 
     */
    public SIAValidationSession(Configuration config, 
    ...
    protected String makeUserQuery(Properties params) {
        ...
        sb.append("POS=").append(params.get("RA")).append(',');
        sb.append(params.get("DEC"));

        sb.append("&SIZE=").append(params.get("RASIZE"));
        String singleSize = params.getProperty("singleSize");
        if (singleSize != null) singleSize = singleSize.toLowerCase();
        String decsize = params.getProperty("DECSIZE");
        if (decsize != null && decsize.length() > 0 && 
            (singleSize == null || ! "true".startsWith(singleSize)))
          sb.append(',').append(decsize);
