...
<xsl:choose>
  <xsl:when test="//OAIValidation or //HarvestValidation or 
                  count(//VOResourceValidation/testQuery) > 1">

<h1>Registry Harvest Service Verification Results</h1>
...
<ul>
   ...
   <li> <a href="#stdoai">Standard OAI-PMH compliance </a> 
        <xsl:apply-templates select="." mode="tocsummary"/> </li>
      </xsl:for-each>

      <xsl:for-each select="//HarvestValidation[1]">
   <li> <a href="#ivoaoai">IVOA Profile on OAI-PMH compliance </a>
        <xsl:apply-templates select="." mode="tocsummary"/> </li>
      </xsl:for-each>
      <xsl:for-each select="//VOResourceValidation[1]">
   <li> <a href="#vorstd">VOResource compliance </a>
        <xsl:apply-templates select="." mode="tocvorsummary"/> </li>
      </xsl:for-each>
</ul>
...
<xsl:apply-templates select="//VOResourceValidation" />
  ...
  <xsl:otherwise>
    <!--  only a single VOResource -->
    <xsl:apply-templates select="//VOResourceValidation" mode="single"/>
  </xsl:otherwise>
</xsl:choose>
...
<xsl:comment> =======================================================================
  ...
  -  ======================================================================= </xsl:comment>
   ...
   <xsl:template match="VOResourceValidation" mode="single" xml:space="preserve">
